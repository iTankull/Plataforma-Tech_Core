  ...
  return (
    ...
    >
      ...
      <div className="absolute top-4 left-4 z-10">
        <span className="bg-text-main text-bg-main px-2.5 py-1 text-[10px] font-black tracking-widest uppercase">
          {product.category}
        </span>
      </div>
      ...
      <div className="p-6 flex flex-col flex-grow">
        ...
        <h3 
          onClick={onClickDetails}
          className="font-sans font-black text-text-main text-xl tracking-tighter hover:text-[#FF3E00] transition-colors duration-200 uppercase line-clamp-1 cursor-pointer mb-2"
        >
          {product.name}
        </h3>

        <p className="text-text-muted text-xs leading-relaxed line-clamp-2 mb-5 flex-grow">
          {product.description}
        </p>
        ...
        <div className="flex flex-wrap gap-1.5 mb-4">
          {Object.entries(product.specs).slice(0, 2).map(([key, val]) => {
            const glossaryMatch = findGlossaryTerm(key) || findGlossaryTerm(val);
            const badge = (
              <span className="text-[9px] font-black tracking-widest uppercase bg-bg-nested text-text-dim px-2.5 py-1 border border-border-very-subtle truncate max-w-[140px] block">
                {key}: {val}
              </span>
            );
            ...
            if (glossaryMatch) {
              return (
                <Tooltip key={key} content={glossaryMatch.explanation} term={glossaryMatch.term}>
                  {badge}
                </Tooltip>
              );
            }
            ...
            return (
              <span key={key} className="text-[9px] font-black tracking-widest uppercase bg-bg-nested text-text-dim px-2.5 py-1 border border-border-very-subtle truncate max-w-[140px]">
                {key}: {val}
              </span>
            );
          })}
        ...
        <div className="mb-5 space-y-2">
          <div className="text-[10px] bg-bg-nested border border-border-subtle text-text-muted p-2.5 font-mono uppercase leading-normal">
            <strong className="text-[#FF3E00] font-black">[ESTADO]:</strong> {product.observations}
          </div>
          <div className="text-[10px] font-mono text-text-muted uppercase flex items-center gap-1.5">
            ESTOQUE DISPONÍVEL: <span className="text-[#FF3E00] font-black bg-bg-nested border border-border-subtle px-2 py-0.5">{product.stock} UNIDADE(S)</span>
          </div>
        </div>

        {/* Pricing details */}
        <div className="border-t border-border-subtle pt-4 flex flex-col gap-1 mb-4">
          <div className="flex items-center gap-2">
            <span className="text-[9px] font-black tracking-widest text-text-dim uppercase">DE:</span>
            <span className="text-sm font-mono text-text-dim line-through">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-3xl font-black italic tracking-tighter text-text-main mt-0.5">
